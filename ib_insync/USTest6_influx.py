...
def Get_Dollar_Bar_Size(dollar_bars, factor=80):
...
def GetTrainTicksInDB(table_name, start_date=datetime.datetime(2019,3,21), end_date=datetime.datetime(2019,5,29)):
    ...
    result = client.query( "select * from " + table_name + " where time>" + ts_start_date + " and time<" + ts_end_date + " order by time asc")  # epoch='ns')
    result

    try:
        df_result = pd.DataFrame(result[table_name.replace('"', '')])
...
def get_panama_stitch_factor(df_old,df_new, dt_last):
    df1 = cleanup_ticks_df(df_old)
    df2 = cleanup_ticks_df(df_new)
        
    df1 = df1.drop(df1.index[df1.Date<dt_last.date()])
    df1 = df1.drop(df1.index[df1.Date>dt_last.date()])
    df2 = df2.drop(df2.index[df2.Date<dt_last.date()])
    df2 = df2.drop(df2.index[df2.Date>dt_last.date()])
    ...
    if len(df1)>len(df2):
        df1 = df1.drop(df1.index[len(df2)])    
    else:
    ...
    old_price = df1.loc[idx]['Price']
    new_price = df2.loc[idx]['Price']
    stitch_factor = new_price.iloc[0] - old_price.iloc[0]
    #todo switch to a loop for each item in idx and store the difference in an array then find the most frequent value
    #find most common number in array and use it as the factor
    #df['item'].value_counts().idxmax() 
    return stitch_factor

def MergeData(df1, df2, stitch_factor):
    df1 = cleanup_ticks_df(df1)
    df2 = cleanup_ticks_df(df2)
    
    if (stitch_factor!=None):
        df1['Price'] = df1['Price'] + stitch_factor
...
#%%
def Test_Stitching():    
    global dollar_bars, df_leftover_ticks, df_original_ticks, bar_size , num_bars_original, train
    # during trainingn set train=true, cont_id = train database table
    # during test and live, set train = false, bar_size=train bar_size, cont_id = live database table
    ...
    if (train):
        ...
        if (stitch):
            ...
            stitch2_factor = get_panama_stitch_factor(cont2_bars, cont3_bars, datetime.datetime(2019,3,15))
            
            
            #now get everything
            cont1_bars = GetTrainTicksInDB(table1,start_date=datetime.datetime(2018,9,26), end_date=datetime.datetime(2019,3,16))
            cont2_bars = GetTrainTicksInDB(table2,start_date=datetime.datetime(2018,9,26), end_date=datetime.datetime(2019,3,16))
            cont3_bars = GetTrainTicksInDB(table3,start_date=datetime.datetime(2018,9,26), end_date=datetime.datetime(2019,3,16))
            ...
            stitch2_train.to_csv(r'c:\test\stitch2_train-' + table3 +'.csv')
        ...
        cont3_bars = GetTrainTicksInDB(table3,start_date=datetime.datetime(2019,3,15), end_date=datetime.datetime(2019,6,26))
        cont4_bars = GetTrainTicksInDB(table4,start_date=datetime.datetime(2019,3,15), end_date=datetime.datetime(2019,6,26))

        stitch3_live = MergeData(cont3_bars,cont4_bars , None)
               #dollar_bars = GetAllTicksInDB(table)
        dollar_bars = cleanup_ticks_df(stitch3_live)
        print(dollar_bars)
        stitch2_train = pd.read_csv(r'c:\test\stitch2_train-' + table3 +'.csv')

        dollar_bars = pd.concat([stitch2_train,stitch3_live])
        print(dollar_bars)
        bar_size = 67936# 44416#49120#42048#71328#53088#63712#50624#63392#55488# 48032#44416# 55680 
